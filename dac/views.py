	...
	if request.method == 'POST':
		...
		if request.user.is_business:
			...
			if google_addr:
				...
				success = get_image_from_address(google_addr["address"],"{0}/business_images/{1}.jpg".format(settings.MEDIA_ROOT,slugify(business_form.data["name"])))
				if success:
					business.image = "business_images/{0}.jpg".format(slugify(business_form.data["name"]))
				else:
